...
ggplot(loadngs3, aes(x = 1, y = names, fill = Loadings)) +
  ...
  theme(axis.text.y = element_text(size = ytextSM),
        axis.text.x = element_text(size = xtextSM, angle = 45,  hjust = 1),
...
ggplot(loadngs3, aes(x = state, y = names, fill = Uniqueness)) +
  ...
  coord_flip() + 
  labs(x = "States", y =  "Parameters from Census") +
  theme(axis.text.y = element_text(size = ytextLG),
        axis.text.x = element_text(size = xtextLG, angle = 45,  hjust = 1),
        strip.text.x = element_text(angle = 90)) +
  scale_fill_gradient(low = "blue", high = "white")
