...
library(knitr)
library(ggh4x)
library(usmap) #for mapping try #2
```
...
re_lableIt <- function(dataa,col){
  ...
  ifelse(dataa[,col] == "EP_UNINSUR", "No Health Insurance",
  ifelse(dataa[,col] == "EP_HBURD"  , "No HS Diploma",
...
heatMapPlot <- function(dataa, aess, colorss, xtext, ytext, fills){
  ggplot(loadngs3, aess) +
  geom_tile(width=1.1, height=.92) +
  geom_text(aes(label = round(fills, 2)),size = inText, vjust = 1) +  # Add this line for text labels
...
tbl <- round(loadingsTable,3) #%>% kable(caption = "Uniquness and Weight by varible on a national level")
loadingsTable$names <- rownames(loadingsTable)
stats <- re_lableIt(loadingsTable,"names")
write.csv(stats,file = paste(savePath,"stats.csv",sep = '/'))
```

```{r heat_map_weights_full}
# Transform the matrix in long 
...
# Sort the unique state names alphabetically
sorted_states <- sort(unique(loadngs3$state), decreasing = T)

# Reapply the factor order to loadngs3
loadngs3$state <- factor(loadngs3$state, levels = sorted_states)
...
write.csv(pctSharedbyStateAll,paste(savePath,"pctSharedbyStateAll.csv",sep = '/'))
```

```{r mappingCorelation_state_level}
pctSharedbyState <- pctSharedbyStateAll[,1:2]
colnames(pctSharedbyState) <- c("state","pct")

plot_usmap(data = pctSharedbyState, values = "pct")+ 
  scale_fill_continuous(
    low = "white",
    high = "purple4",
    name = "Percent Shared", label = scales::comma)+
  labs(title = "Percent Shared Between State level FA and CDC SVI in top 10% ranked of each stratified by state") +
  theme(legend.position = "right",plot.background = element_rect(fill = "white"))

saveAs("statelevelmap.png",sizeLG)
```
...
plot_usmap(data = pctSharedbyState, values = "pct")+ 
  scale_fill_continuous(
    ...
    high = "darkgreen",
    name = "Percent Shared", label = scales::comma)+
  labs(title = "Percent Shared Between National level FA and CDC SVI in top 10% ranked of each stratified by state") +
  theme(legend.position = "right",plot.background = element_rect(fill = "white"))

saveAs("nationllevelmap.png",sizeLG)
```
